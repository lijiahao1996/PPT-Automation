...
- **用途**：适合排名对比
- **数据结构**：需要 2 列：1个分类字段 + 1个数值字段
- **示例**：销售员 | 总销售额
- **字段映射**：y_field=分类列, x_field=数值列

#### bar_vertical - 纵向柱状图
- **用途**：适合指标对比
- **数据结构**：需要 2 列：1个分类字段 + 1个数值字段
- **示例**：指标名称 | 指标值
- **字段映射**：x_field=分类列, y_field=数值列

#### pie - 环形饼图
- **用途**：适合占比分析
- **数据结构**：需要 2 列：1个分类字段 + 1个数值字段
- **示例**：产品类别 | 销售额
- **字段映射**：category_field=分类列, value_field=数值列

#### line - 折线图
- **用途**：适合趋势分析
- **数据结构**：需要 2+ 列：1个时间字段 + 1或多个数值字段
- **示例**：年月 | 总销售额, 订单数
- **字段映射**：x_field=时间列, y_field=数值列（可多个）

#### column_clustered - 多列柱状图
- **用途**：适合多系列对比
- **数据结构**：需要 3+ 列：1个分类字段 + 2或多个数值字段
- **示例**：性别 | 平均年龄, 平均月薪, 平均消费次数
- **字段映射**：category_field=分类列, series=数值列列表

#### heatmap - 热力图
- **用途**：适合矩阵分析（二维交叉）
- **数据结构**：需要 3+ 列：1个行分类 + 多个列分类（会自动透视）
- **示例**：销售员 | 产品A, 产品B, 产品C
- **字段映射**：y_field=行分类列
...
- **数据结构**：需要 2 列：2个数值字段
- **示例**：单价 | 销量
- **字段映射**：x_field=X轴数值, y_field=Y轴数值

#### histogram - 直方图
- **用途**：适合分布分析
- **数据结构**：需要 1 列：1个数值字段（使用原始数据，不是统计汇总）
- **示例**：年龄
- **字段映射**：field=数值列
...
- **数据结构**：需要 3 列：3个数值字段
- **示例**：市场份额 | 增长率 | 营收规模
- **字段映射**：x_field=X轴数值, y_field=Y轴数值, size_field=气泡大小

#### area - 面积图
- **用途**：适合累计趋势
- **数据结构**：需要 2+ 列：1个时间字段 + 1或多个数值字段
- **示例**：月份 | 产品收入, 服务收入
- **字段映射**：x_field=时间列, y_field=数值列列表

#### errorbar - 误差棒图
- **用途**：适合误差分析
- **数据结构**：需要 3 列：1个分类 + 1个数值 + 1个误差
- **示例**：实验组 | 平均值 | 标准差
- **字段映射**：x_field=分类列, y_field=数值列, error_field=误差列

#### polar - 极坐标图
- **用途**：适合周期数据
- **数据结构**：需要 2 列：1个角度字段 + 1个半径字段
- **示例**：月份角度 | 销售额
- **字段映射**：x_field=角度列, y_field=半径列

#### waterfall - 瀑布图
- **用途**：适合增减分析
- **数据结构**：需要 2 列：1个分类字段 + 1个数值字段（正负）
- **示例**：项目 | 变动金额
- **字段映射**：category_field=分类列, value_field=数值列

#### funnel - 漏斗图
- **用途**：适合流程转化
- **数据结构**：需要 2 列：1个阶段字段 + 1个数值字段（递减）
- **示例**：阶段 | 用户数
- **字段映射**：category_field=阶段列, value_field=数值列
